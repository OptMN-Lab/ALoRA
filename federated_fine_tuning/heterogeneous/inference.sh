...
CUDA_VISIBLE_DEVICES=0 nohup python GlobalModel_generated.py \
      --load_8bit \
      --base_model 'meta-llama/Llama-2-7b-hf' \
      --lora_weights_path "${model_path}/8/10/local_output_${client_id}/pytorch_model.bin" \
      --lora_config_path "${model_path}/8/10/local_output_${client_id}/"   \
      --lora_base_weights_path "${model_path}/8/9/adapter_model.bin" \
